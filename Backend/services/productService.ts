...
export const updateAmountProductSevice = async function (data: IProduct) {
  try {
    const itemUpdateProduct = await Product.findOne({
      _id: new mongoose.Types.ObjectId(data._id),
    });
    if (itemUpdateProduct) {
      (itemUpdateProduct.amount = data.amount), await itemUpdateProduct.save();
      return okResponse(itemUpdateProduct);
    } else {
      return dataNotFoundResponse();
    }
...
export const findByPageService = async function (data: any) {
  try {
    ...
    const itemFind = await Product.find(condition)
      .sort({ createdAt: -1 })
